...
const ACTIONS = ["confirm_hq_approval", "create_student_record", "activate_student", "correct_student_number"];
...
Deno.serve(async (req) => {
  ...
  if (error) {
    ...
    if (/reason required|student number required/i.test(error.message)) return fail(400, "bad_request");
