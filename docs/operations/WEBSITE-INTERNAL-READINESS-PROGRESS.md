...
复现方式：把 `CONFIG.formEndpoint` 指向 `http://127.0.0.1:1/`（本机必然拒绝连接），
不碰任何真实服务。然后按**真人最常见的做法**操作 —— 把「提交申请」刚滚出来就点：

```
点击前：按钮在卡片可视区内 = true
点击后：状态区在卡片可视区内 = false   状态区整体在可视区下方 = true   被裁掉 = 38px
```

申请弹窗的 `.modal-card` 自己带滚动条（`max-height:90vh;overflow:auto`），
而状态区排在提交按钮**下面**。按钮贴着可视区底边时，38px 的错误提示整条落在可视区之外。
**提交失败 → 按钮恢复原样 → 一个字的反馈都没有。** 访客多半会反复再点。

换一个稍高的滚动位置（按钮居中）也还差一点：提示有 9px 被卡片裁掉。
也就是说这不是某个极端位置的巧合，而是「状态区在按钮下方 + 卡片自带滚动」的必然结果。

## 118. 成功之后，旧结果一直挂着

咨询表单实测：提交成功显示「已收到。当前演示版将咨询保存在本机浏览器中。」，
随后**在姓名框里重新输入**，这句话仍然挂在那里 —— 它描述的是上一次提交，
却贴在这一次的输入旁边。
...
// 结果提示只描述「刚刚那一次提交」。一有输入就清掉，让状态区要么空、要么是当前这次的
...
申请与咨询两个表单的成功/失败四条分支都调 `revealStatus`，两个表单都挂
`freshenStatusOnEdit`。没有改任何文案，没有改提交逻辑，没有动 `aria-live`
（提示仍由 `polite` 区播报，焦点不被抢走 —— 用户可能正要再点一次提交）。

## 120. 回归扩到 15/15

`scripts/test-application-flow.mjs` 新增三条：

| 断言 | 内容 |
|---|---|
| A12 | 失败提示被带进卡片可视区（被裁掉 0px），弹窗不关、可以重试 |
| A13 | 一改表单，上一次的提交结果立刻清掉 |
| A14 | **负向控制之二**：把 `revealStatus` 换成空操作，提示重新落到可视区外（实测被裁掉 38px） |

A14 直接改站点自己的函数而不是换一套宽松断言 —— 不会变红的负向控制等于没有控制。
...
| 观察 | 结论 |
|---|---|
| 咨询表单空提交 | 焦点落到第一个未填字段，原生校验正常 |
| 手机上错误文案被按钮挤扁 | 实测 347px 行里按钮 103 + 文案 228x38 两行，不溢出，可读 |
| 失败后表单内容丢失 | 内容完整保留，按钮恢复可点，`aria-busy=false` |
...
| 15 | `3faa1da` | discover 测验换题/换视图焦点 + 进度语义 | `test-discover-flow` 14/14 |
| 16 | `db3a8c1` | 分支上线验收清单汇总（仅文档） | — |
| 17 | `<本轮>` | discover 原型详情：12 项键盘不可达 + 开关焦点 | `test-discover-flow` D9–D12 / D0b |

**合计 13 套浏览器/契约回归、约 245 项断言，末次全量执行全绿。**
...
- 资源中心搜索框 `.resource-search-wrap` 带 `hidden` 且全站无代码解除（死 UI），
  是启用还是移除属产品决定。
- ~~`discover.html` 的原型详情视图~~ —— 已在第二十一轮完成。
- `#detail` 的左右切换按钮（`stepDetail`）与结果页缺少进入详情的入口，属产品取舍，未动。

---

# 第二十一轮：discover 原型详情视图

## 131. 12 个原型对键盘访客根本不存在

首屏原型条的每一项是带 `onclick` 的 `<img>` —— 图片不可聚焦、回车没有反应。
真实键盘实测：

```
Tab 2 次，命中原型条 = true
途经焦点: button.gold-btn "开始快速探索 →" → div#strip.strip "" @landing
```

**焦点停在的是 `.strip` 这个容器**，不是任何一项。Chrome 会让可横向滚动的容器可聚焦，
于是「焦点进得去」和「原型打得开」看起来像一回事 —— 实际差着一整块内容：
12 个原型的倾向说明、优势、盲点、常见侍奉与装备方向，键盘与读屏访客一项也到不了。

这正是上一轮那条经验的另一面：**只看「Tab 能不能到达这块区域」会给出假绿**，
必须看焦点落在的是不是**可激活的那个元素**。

## 132. 打开 / 返回详情，焦点同样掉回 body
...
与第二十轮修的是同一类，但 `openDetail` / `closeDetail` 不在上一轮的改动范围内。

## 133. 修法

把原型条的每一项换成真 `<button>`，与详情页 `#dGrid` 里**本来就有的写法**保持一致：

```js
ARCH.forEach(function(a){
  var b = document.createElement('button');
  b.type = 'button';
  b.setAttribute('aria-label', a.l + ' · 查看倾向说明');
  var im = document.createElement('img');
  im.src = img(a.k); im.alt = ''; im.loading = 'lazy';   // 名称由按钮给，图片不再重复播报
  b.appendChild(im);
  b.onclick = function(){ openDetail(a.k); };
  strip.appendChild(b);
});
```

外观整体从 `.strip img` 搬到 `.strip button`（宽度、圆角、边框、阴影、滚动吸附都照搬），
图片改成 `width:100%;display:block`，**视觉与改前一致**；按钮另加 `:focus-visible` 焦点圈。

焦点接管沿用第二十轮的 `focusQuietly`：`openDetail()` 末尾聚焦 `#dTitle`
（加 `tabindex="-1"` + `outline:none`），`closeDetail()` 按来处分别交回
「开始快速探索」或结果页的结论标题。

## 134. 回归扩到 14/14

`scripts/test-discover-flow.mjs` 新增：

| 断言 | 内容 |
|---|---|
| D9 | 原型条 12 项是真按钮，键盘可逐个聚焦，且有可读的无障碍名称 |
| D10 | 键盘回车打开详情，焦点进入详情内容 |
| D11 | 详情内切换原型：标题更新，焦点仍在详情里 |
| D12 | 从详情返回首屏，焦点交回「开始快速探索」 |
| D0b | **负向控制之二**：把原型条换回 `<img>`，键盘 20 次 Tab 内到不了任何一项 |

D0b 直接还原站点原来的写法，而不是换一套宽松断言。

**附带回归无回归**：13 套全绿（`test-discover-back-link` 30/30 起）。

## 135. 顺带查清、不是缺陷的两处

- 详情页内点缩略图切换原型时，`#dGrid` 里的项**本来就是真 `<button>`**，
  鼠标点击后焦点正常留在按钮上；本轮只是把焦点进一步带到标题，让读屏访客从新标题读起。
