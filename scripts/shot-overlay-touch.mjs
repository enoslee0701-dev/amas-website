#!/usr/bin/env node
// 打开态浮层前后对照截图（抽屉与客服面板）。--revert 用注入还原样式跑同一条代码路径，产出「修复前」。
//
...
const REVERT = `(() => {
  ...
  st.textContent =
    ...
    '.apply-btn{min-height:0!important;display:inline-block!important}' +
    '.chat-head button{min-width:0!important;min-height:0!important;padding:4px!important;display:inline!important}' +
    '.chat-chips button{min-height:0!important;display:inline!important;padding:7px 13px!important}' +
    '.chat-input input,.chat-input button{height:42px!important;min-width:0!important}' +
    '.promo-close{min-width:0!important;min-height:0!important;padding:4px 8px!important;display:inline!important}' +
...
try {
  ...
  for (const v of [{ w: 320, h: 720, mobile: true }, { w: 375, h: 780, mobile: true },
    ...
    // 用真人的入口打开客服面板（它同时包含关闭键、快捷问句、输入框与发送键，
    // 一张图能覆盖本轮改的大部分控件）
    await cdp.ev(`document.querySelector('#chatFab').click()`);
    await sleep(800);
